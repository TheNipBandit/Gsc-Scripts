// Instantly spawn all perks at once and play the incoming sound

// Spawns all perks to fall from the sky, with effects and animation
spawn_all_perks_instant()
{
    // Use the same struct logic as the map for correct animation and sound
    top_height = 8000;
    machines = [];
    triggers = [];

    // Use the structs for perk machines (handles .original_pos, .original_angles, etc)
    if ( isdefined(level.random_perk_structs) )
    {
        for ( i = 0; i < 5; i++ )
        {
            machines[i] = level.random_perk_structs[i];
            triggers[i] = getent( machines[i].target, "targetname" );
        }
    }
    else
    {
        // Fallback to nuked_perks if random_perk_structs not set
        for ( i = 0; i < 5; i++ )
        {
            machines[i] = level.nuked_perks[i];
            triggers[i] = getent( machines[i].target, "targetname" );
        }
    }

    // Move all perks up and turn off triggers
    for ( i = 0; i < machines.size; i++ )
    {
        if ( isdefined(machines[i]) && isdefined(triggers[i]) )
        {
            move_perk( machines[i], top_height, 5.0, 0.001 );
            triggers[i] trigger_off();
        }
    }

    wait(2.0); // Give time for perks to move up

    // Bring each perk down, 5 seconds apart, using the full animation logic
    for ( i = 0; i < machines.size; i++ )
    {
        if ( isdefined(machines[i]) && isdefined(triggers[i]) )
        {
            bring_perk( machines[i], triggers[i] );
            wait(5.0);
        }
    }
}

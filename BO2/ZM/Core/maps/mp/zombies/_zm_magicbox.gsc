...
treasure_chest_think()
{
    ...
    if ( isdefined( self.joker_no_move ) && self.joker_no_move )
    {
        self.grab_weapon_hint = 0;
        thread maps\mp\zombies\_zm_unitrigger::unregister_unitrigger( self.unitrigger_stub );

        if ( isdefined( self.chest_lid ) )
            self.chest_lid thread treasure_chest_lid_close( 1 );
    ...
    else if ( flag( "moving_chest_now" ) && !level.zombie_vars["zombie_powerup_fire_sale_on"] && !self._box_opened_by_fire_sale )
...
#/
    ...
    // If this was a restricted category roll and player owns all valid weapons,
    // force a teddy display without moving the box.
    if ( isdefined( level.custom_magic_box_selection_logic ) )
    {
        level._magic_box_joker_nomove = 1;
        return undefined;
    }

    // Hard fallback: still respect current box lock/pool restrictions.
    for ( i = 0; i < keys.size; i++ )
    {
...
treasure_chest_weapon_spawn( chest, player, respin )
{
    ...
    if ( isdefined( level._magic_box_joker_nomove ) && level._magic_box_joker_nomove )
    {
        level._magic_box_joker_nomove = undefined;
        chest.joker_no_move = 1;
        rand = undefined;
    }
    ...
    else
    {
        ...
        if ( isdefined( rand ) && ( rand == "tesla_gun_zm" || rand == "ray_gun_zm" ) )
        {
            if ( rand == "ray_gun_zm" )
        ...
        if ( !chest.timedout && isdefined( rand ) )
        {
            if ( isdefined( self.weapon_model ) )
                self.weapon_model delete();

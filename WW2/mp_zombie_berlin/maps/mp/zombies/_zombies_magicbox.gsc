...
func_A7E6(param_00)
{
	...
	// func_A7E6: wallbuy chalk prices only. Shepherd wall slider -> level.override_weapon_costs (below).
...
//Function Number: 95
// Shepherd mystery box cost: Game Settings slider -> level.LobbyMods_mystery_box_price_override (flat jolts; no round ladder, no discount perk mult).
// func_43FF is what stock uses for box debit + UI cost. Same script also has wallbuy in func_A7E6.
func_43FF(param_00)
{
	if(maps\mp\_utility::func_3FA0("fire_sale"))
	{
	...
	if(isdefined(level.LobbyMods_mystery_box_price_override) && level.LobbyMods_mystery_box_price_override > 0)
	{
		return int(level.LobbyMods_mystery_box_price_override);
	}
	...
	if(isplayer(param_00) && param_00 lib_0573::func_4B7B() && isdefined(param_00.var_65EC))
	{
